...
process.env.MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/dnd-tracker-test';
      ...
      return {
        find: jest.fn(async (filter = {}) => {
          ...
          if ('deletedAt' in filter) {
            return docs.filter((d) => (filter.deletedAt === null ? d.deletedAt === null : d.deletedAt !== null));
          }
          return docs.filter((d) => Object.entries(filter).every(([k, v]) => d[k] === v));
        }),
        findById: jest.fn(async (id) => docs.find((d) => d._id === id) || null),
        findOne: jest.fn(async (filter = {}) => docs.find((d) => Object.entries(filter).every(([k, v]) => d[k] === v)) || null),
        create: jest.fn(async (payload) => {
          ...
          if (name === 'User') {
            ...
            if ('userId' in doc) {
              Object.defineProperty(doc, 'userId', { writable: false, enumerable: true });
            }
            if ('email' in doc) {
              Object.defineProperty(doc, 'email', { writable: false, enumerable: true });
            }
          }
        ...
        updateOne: jest.fn(async (_filter, _update) => ({ matchedCount: 1, modifiedCount: 1 })),
