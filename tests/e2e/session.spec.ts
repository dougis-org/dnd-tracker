...
test.describe('Session Persistence (E2E)', () => {
  ...
  test('should handle session refresh correctly', async ({ page }) => {
    ...
    const response = await page.request.get('http://localhost:3002/api/auth/session')
  ...
  test('should handle sign-out endpoint correctly', async ({ page }) => {
    ...
    const response = await page.request.post('http://localhost:3002/api/auth/sign-out')
