...
- **OfflineQueueEntry**: Representation of queued user actions while offline (operation type, payload, timestamp, retry count) persisted locally until reconnection. Persisted in `IndexedDB` as the primary persistent store for queued entries.
...
- Q: Storage for `OfflineQueueEntry` → A: `IndexedDB` (primary persistent store).
- Q: Cache eviction policy → A: Hybrid (LRU + size cap).
- Q: Controlled update safe window → A: Safe window (24 hours).
- Q: API response TTL → A: 5 minutes (short TTL for network-first endpoints).
- Q: Network-first scope → A: Only mutable game-state and user-specific endpoints are network-first.
- Q: Cache size cap → A: 50 MB.
- Q: Sensitive-data caching policy → A: Strict — never cache any user-specific or personal data (auth tokens, PII, private inventories, session data).
- Q: Telemetry reporting → A: Defer to analytics plan (decide during analytics/implementation).
- Q: Service worker token handling → A: Service worker must not access/attach auth tokens; client attaches them.
- Q: Offline queue retry policy → A: Exponential backoff up to 5 attempts, then surface failure to user.
- Q: Cache eviction mechanism → A: LRU by last-access time (approximate) + size cap.
- Q: Cache update strategy → A: Use stale-while-revalidate for static assets (serve cache, update in background).
- Q: Cache versioning strategy → A: Content-hash-based filenames (auto invalidated when build output changes).
- Q: Offline UI for connectivity → A: Non-blocking banner with short message, `Retry` button, dismiss control, and compact sync-progress indicator.
- Q: Controlled update activation behavior → A: Show an "Update available" banner and wait for the user's refresh; auto-activate after the safe window (24 hours).
- Q: Offline queue ordering → A: FIFO — process queued offline operations in the order enqueued.
- Q: Offline queue encryption → A: Encrypt `OfflineQueueEntry` payloads at rest using Web Crypto (AES-GCM) for PII/sensitive fields.
