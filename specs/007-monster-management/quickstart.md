...
1. The feature uses a small mock adapter under `src/lib/mocks/monsters` (implementations vary). For now, run the dev server; the UI will use in-repo mock fixtures under `specs/007-monster-management/fixtures` if present.
...
- Add Zod schemas in `src/lib/schemas/monster.ts` and reference them from forms and component tests.  
