...
# Use build secrets to access environment variables during build
RUN --mount=type=secret,id=NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY \
    ...
    export NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY=$(cat /run/secrets/NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY || echo "") && \
    export CLERK_SECRET_KEY=$(cat /run/secrets/CLERK_SECRET_KEY || echo "") && \
    export MONGODB_URI=$(cat /run/secrets/MONGODB_URI || echo "") && \
    npm run build
